package com.wxx.gulimall.cart.service.Impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.TypeReference;
import com.wxx.common.utils.R;
import com.wxx.gulimall.cart.feign.ProductFeignService;
import com.wxx.gulimall.cart.interceptor.CartInterceptor;
import com.wxx.gulimall.cart.service.CartService;
import com.wxx.gulimall.cart.vo.Cart;
import com.wxx.gulimall.cart.vo.CartItem;
import com.wxx.gulimall.cart.vo.SkuInfoVO;
...
public class CartServiceImpl implements CartService {
    ...
    public Cart getCart() throws ExecutionException, InterruptedException {
        ...
        if(userInfoDTO.getUserId() != null){
            ...
            if(tempItem != null){
                // 1.2 临时购物车有数据 则进行合并
                log.info("\n[" + userInfoDTO.getUsername() + "] 的购物车已合并");
    ...
    public BigDecimal toTrade() throws ExecutionException, InterruptedException {
        ...
        UserInfoDTO userInfoDTO = CartInterceptor.threadLocal.get();
        stringRedisTemplate.delete(CART_PREFIX + (userInfoDTO.getUserId() != null ? userInfoDTO.getUserId().toString() : userInfoDTO.getUserKey()));
        return amount;
    }
    ...
    private BoundHashOperations<String, Object, Object> getCartOps() {
        ...
        if(userInfoDTO.getUserId() != null){
            log.debug("\n用户 [" + userInfoDTO.getUsername() + "] 正在操作购物车");
            // 已登录的用户购物车的标识
            cartKey += userInfoDTO.getUserId();
        }else{
            log.debug("\n临时用户 [" + userInfoDTO.getUserKey() + "] 正在操作购物车");
            // 未登录的用户购物车的标识
            cartKey += userInfoDTO.getUserKey();
        }
